...
export async function createCollection(name: string, description?: string): Promise<Collection> {
  return invoke<Collection>('create_collection', { name, description: description || null });
}

export async function updateCollection(id: number, name: string, description?: string): Promise<Collection> {
  return invoke<Collection>('update_collection', { id, name, description: description || null });
}
...
// Diagrams API
export async function getDiagramsByCollection(collectionId: number): Promise<Diagram[]> {
...
export async function createDiagram(collectionId: number, name: string, content: string): Promise<Diagram> {
  return invoke<Diagram>('create_diagram', { collectionId, name, content });
}

export async function updateDiagram(id: number, name: string, content: string): Promise<Diagram> {
